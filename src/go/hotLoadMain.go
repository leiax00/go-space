...
import (
	"fmt"
	"github.com/lua_go/base/hotload"
	"sync/atomic"
	"time"
)
...
func (a *AppConfigMgr) Callback(conf *hotload.Config) {
	appConfig := &AppConfig{}
...
func initConfig(file string) {
	// [1] 打开配置文件
	conf, err := hotload.NewConfig(file)
	if err != nil {
...
func main() {
	confFile := "../resources/test-hotload.properties"
	initConfig(confFile)
